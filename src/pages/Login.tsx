...
export function Login() {
  ...
  async function onSetup(e: React.FormEvent) {
    ...
    setBusy(true)
    try {
      const ok = await initAdminAccount(setupEmail, setupPassword)
      if (!ok) {
        setError('Could not create account. Try again.')
        return
      }
      const logged = await adminLogin(normalizeEmail(setupEmail), setupPassword)
      if (logged) navigate('/admin', { replace: true })
    } finally {
      setBusy(false)
    }
  }
